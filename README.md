...
1. [x] Create a folder named `library` in your working directory.

2. [x] Inside the `library` folder, create a file named `__init__.py`. This file is required for Python to treat the directory as a package.

3. [x]Create a new file named `librarian.py` inside the `library` folder. In this file, define the following functions:
...
4. [x] Write a script named `main.py` in your working directory (outside the `library` folder) that imports and uses the `librarian` module from the `library` package to manage a simple library system.

5. [x] use the function from librarian to add books, remove book, checout a book, and display books .
